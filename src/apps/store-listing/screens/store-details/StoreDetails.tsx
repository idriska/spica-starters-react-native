    ...
    <View style={styles.container}>
      ...
      {activeIndex === 'info' && (
        <View style={{padding: 10}}>
          ...
          <Text>
            <Text style={{fontWeight: '800'}}>Phone:</Text> {store.phone}
          </Text>
          <Text>
            <Text style={{fontWeight: '800'}}>Email:</Text> {store.mail}
          </Text>
          <Text>
            <Text style={{fontWeight: '800'}}>Address:</Text> {store.adress}
          </Text>
          <View
            style={{
              borderTopWidth: 2,
          ...
          {store.working_hours.length &&
            store.working_hours.map((el, index) => {
              return (
                <Text key={index} style={{textAlign: 'center'}}>
                  <Text style={{fontWeight: '800'}}>{days[index]}:</Text>{' '}
                  {store.working_hours[index].hours[0].from +
                    ' - ' +
                    store.working_hours[index].hours[0].until}
                </Text>
              );
            })}
        </View>
